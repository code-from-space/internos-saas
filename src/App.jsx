        ...
        <header className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-bold text-gray-800">
              {isHR ? 'Global Fleet View' : 'My Portal'}
            </h2>
            <p className="text-gray-500 mt-1">
              {isHR ? 'Monitor all active engineering interns' : 'Manage your tasks and progress'}
            </p>
          </div>
          <div className="flex space-x-4">
            <button 
              ...
              className="text-gray-500 hover:text-gray-700 font-medium px-4 py-2 cursor-pointer transition-colors"
            >
              Sign Out
            </button>
            
            {/* ONLY show the Invite button to HR! */}
            {isHR && (
              <button 
        ...
        {isHR ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <StatCard title="Tasks Completed (This Week)" value="35" valueColor="text-green-600" />
            </div>
            ...
            <div className="mb-6">
              <input
                type="text"
                placeholder="Search by name or role (e.g., 'Frontend')..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full md:w-96 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none shadow-sm transition-all"
              />
            </div>
            ...
            <InternTable 
              ...
              onAuditIntern={setAuditIntern} // <-- Pass the function down!
            />
          </>
        ) : (
